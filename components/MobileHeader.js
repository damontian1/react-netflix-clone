...
export default function MobileHeader() {
  ...
  const isShowPage = router.pathname.includes("/media");
  ...
  if (!isShowPage) {
      ...
      <header className="text-white absolute px-5 z-40 py-5 w-full">
        <nav>
          <div className="flex justify-between items-baseline mb-2">
            <Link href="/">
              <img src={`${process.env.assetPrefix}logo-small.png`} alt="" className="w-7" />
            </Link>
            <div className="flex space-x-2">
              ...
              <button className="px-3 flex space-x-1 items-center focus:outline-none flex-shrink-0">
                <img src={`${process.env.assetPrefix}damon.jpeg`} alt="" className="w-6 h-6 rounded-full" />
              </button>
            </div>
          </div>
    ...
    return (
      <header>
        <div className="absolute top-0 right-0 p-4 text-white z-50">
          <button
            onClick={() => router.back()}
            className="bg-white rounded-full p-3 focus:outline-none">
            <svg viewBox="0 0 14 14" className="stroke-2 stroke-current text-black w-4">
